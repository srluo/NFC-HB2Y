...
function sign({ uid, ts }) {
  ...
  let key = [], iv = [], keystream = [], R = [], S = [];
  let key_size = 80, iv_size = 32, keystream_size = 4;
  ...
  function CLOCK_S(input_bit, control_bit) {
    ...
    if (Feedback_bit) {
      if (control_bit) {
        S[0] ^= S_Mask1[0]; S[1] ^= S_Mask1[1]; S[2] ^= S_Mask1[2];
      } else {
        S[0] ^= S_Mask0[0]; S[1] ^= S_Mask0[1]; S[2] ^= S_Mask0[2];
      }
    }
  }
  ...
  function iv_setup() {
    R = [0, 0, 0]; S = [0, 0, 0];
    for (let i = 0; i < Math.floor(key_size / 8); i++) key[i] = reverse_bit(rawKey[9 - i]);
    for (let i = 0; i < Math.floor(iv_size / 8); i++) iv[i] = reverse_bit(rawIv[3 - i]);
    for (let i = 0; i < iv_size; i++) CLOCK_KG(1, (iv[Math.floor(i / 8)] >>> (7 - (i % 8))) & 1);
    for (let i = 0; i < key_size; i++) CLOCK_KG(1, (key[Math.floor(i / 8)] >>> (7 - (i % 8))) & 1);
  ...
  return keystream.map(c => ("00" + c.toString(16)).slice(-2)).join("").toUpperCase().substring(0, 8);
}
