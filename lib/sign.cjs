...
function sign({ uid, ts, prefix = "FFFFFF" }) {
  ...
  let key_size = 80;
  let iv_size = 32;
  let keystream_size = 4; // 4 byte = 8 hex char
  let R = [0, 0, 0];
  let S = [0, 0, 0];
  ...
  function iv_setup() {
    ...
    for (let i = 0; i < key_size / 8; i++) {
      key[i] = reverse_bit(rawKey[9 - i]);
    }

    for (let i = 0; i < iv_size / 8; i++) {
  ...
  for (let i = 0; i < keystream_size; i++) {
    str += ("00" + keystream[i].toString(16)).slice(-2);
  }
  return str.toUpperCase(); // ← 回傳 8 hex
}
